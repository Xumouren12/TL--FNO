...
class DataSet:
    ...
    def load_data(self):
        ...
        file = scio.loadmat('Dataset_equilateral_crack_try.mat')
...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2, width):
        ...
        self.ft_fc5 = nn.Linear(625, 2295)
...
################################################################
dict1 = torch.load("D:\Transfermodels\TL1-3_source")
...
for ep in range(epochs):
    ...
    with torch.no_grad():
        for x, y in test_loader:
            x, y = x.cuda(), y.cuda()
            out = model(x)
            out = torch.reshape(out, [batch_size2, 2295, 1])
            y = torch.reshape(y, [batch_size2, 2295, 1])
