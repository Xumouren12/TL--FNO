...
class DataSet:
    ...
    def load_data(self):
        ...
        file = scio.loadmat('Dataset_1crack_try.mat')
...
for ep in range(epochs):
    ...
    with torch.no_grad():
        for x, y in test_loader:
            x, y = x.cuda(), y.cuda()
            out = model(x)
            out = torch.reshape(out, [batch_size2, 1538, 1])
            y = torch.reshape(y, [batch_size2, 1538, 1])
...
t3 = default_timer()
print(t3-t0)
# torch.save(model.state_dict(), "D:\Transfermodels\TL4_source")