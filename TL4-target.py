...
class DataSet:
    ...
    def load_data(self):
        ...
        file = scio.loadmat('Dataset_2crack_try.mat')
...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2, width):
        ...
        self.ft_fc5 = nn.Linear(625, 1552)

        dict = torch.load("D:\Transfermodels\TL4_source_lambda")
    ...
    def forward(self, x):
        ...
        x_f3_all = x_f3 + x_f2 * self.lambda3 + x_f1 * self.lambda4
        ...
        x_f4_all = x_f4 + x_f3 * self.lambda6 + x_f2 * self.lambda7 + x * self.lambda9
...
sampler = np.random.choice(range(2100), size=ntest, replace=False)
...
################################################################
# dict1 = torch.load("D:\Transfermodels\TL4_source")
...
# reg_loss = Regularization(model, weight_list, 1e-2)
...
for ep in range(epochs):
    ...
    with torch.no_grad():
        for x, y in test_loader:
            x, y = x.cuda(), y.cuda()
            out = model(x)
            out = torch.reshape(out, [batch_size2, 1552, 1])
            y = torch.reshape(y, [batch_size2, 1552, 1])
