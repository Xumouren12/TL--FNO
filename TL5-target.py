...
class DataSet:
    ...
    def load_data(self):
        # Source data
        file = scio.loadmat('Dataset_TL5_target_try.mat')
        s_bc = 101
        s = 1183
...
class FNO1d(nn.Module):
    def __init__(self, modes, width):
        ...
        self.ft_fc3 = nn.Linear(101, 1183)
...
for ep in range(epochs):
    ...
    with torch.no_grad():
        for x, y, z in test_loader:
            ...
            out1 = out[:, :, 0]
            out2 = out[:, :, 1]
            out1 = torch.reshape(out1, [batch_size2, 1183, 1])
            out2 = torch.reshape(out2, [batch_size2, 1183, 1])
            y = torch.reshape(y, [batch_size2, 1183, 1])
            z = torch.reshape(z, [batch_size2, 1183, 1])
